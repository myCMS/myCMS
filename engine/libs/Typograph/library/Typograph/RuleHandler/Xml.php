 ...
 */
require_once TYPOGRAPH_LIBRARY_ROOT.'Typograph/RuleHandler/Abstract.php';
...
class Pride_Typograph_RuleHandler_Xml extends Pride_Typograph_RuleHandler_Abstract
{
	...
	protected function _checkXmlFile($path)
	{
		if (!is_string($path) || empty($path)) {
			throw new ExceptionExt('Bad regex file name');
		}
		
		if (!is_readable($path)) {
			throw new ExceptionExt("File '{$path}' isn't readable");
		}
		
		if (!filesize($this->_options['xml_regex_file'])) {
			throw new ExceptionExt("File '{$path}' is empty");
		}
	}
}