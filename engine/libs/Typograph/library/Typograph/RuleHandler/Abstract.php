...
abstract class Pride_Typograph_RuleHandler_Abstract
{
    ...
    public function addSafetyBlock($before, $after, $unquote = false)
    {
        foreach ($this->_safetyBlocks as $block) {
            if (in_array($before, $block) || in_array($before, $after)) {
                throw new ExceptionExt("'$before' or '$after' already exists in safety blocks");
            }
        }
        
        if ($before === '<' || $after === '>') {
            throw new ExceptionExt("Bad '$before' or '$after'");
        }
    ...
    {
        if (!is_string($text)) {
            throw new ExceptionExt('Incorrect data');
        }
        
        $text = trim($text);
        
        if ('' === $text) {
            throw new ExceptionExt('String is empty');
        }
    ...
    {
        if (!is_bool($value)) {
            throw new ExceptionExt("Access key '$key' must be boolean");
        }
    ...
    {
        if (!count($options)) {
            throw new ExceptionExt('Options array is empty');
        }
    ...
    {
        if (!is_bool($safe)) {
            throw new ExceptionExt('Incorrect var type');
        }
